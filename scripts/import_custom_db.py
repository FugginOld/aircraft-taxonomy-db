...
6. Move any review-required rows to ``review/aircraft-taxonomy-custom-db_review.csv``
   (uses a counter suffix if that file already exists to avoid clobbering prior backlog).
...
def _unique_dest(review_dir: Path, name: str) -> Path:
    """Return a path in *review_dir* that does not already exist.

    If ``name`` is free it is returned as-is.  Otherwise a numeric suffix
    (-1, -2, …) is appended before the extension until a free slot is found.
    """
    stem = Path(name).stem
    ...
    while candidate.exists():
        candidate = review_dir / f"{stem}-{counter}{suffix}"
        counter += 1
    return candidate
...
def main(argv: list[str] | None = None) -> int:
    ...
    # ------------------------------------------------------------------
    # Guard: main database must exist and be readable
    # ------------------------------------------------------------------
    if not main_path.exists():
        logger.error("Main database not found: %s", main_path)
        return 1
    if not main_path.is_file():
        logger.error("Main database path is not a file: %s", main_path)
        return 1
    ...
    try:
        ...
        logger.error("Failed to read main database %s: %s", main_path, exc)
        return 1
    ...
    if review_rows:
        ...
        dest = _unique_dest(review_dir, review_path.name)
